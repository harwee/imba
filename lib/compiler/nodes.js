...
TagDeclaration.prototype.js = function (o){
	...
	var js = ("" + mark + (this.scope__().imba().c()) + "." + meth + "(" + params.join(', ') + ")");
...
Ivar.prototype.c = function (){
	return '_' + helpers.dashToCamelCase(this._value).slice(1); // .replace(/^@/,'') # AST.mark(@value) +
};
...
TAG_TYPES.HTML_OPT = "abbr address area article aside audio b base bdi bdo big blockquote body br button canvas caption cite code col colgroup dd del details dfn div dl dt em embed fieldset figcaption figure footer form h1 h2 h3 h4 h5 h6 head header hr html i img ins kbd keygen label legend li link main mark meta meter nav noscript object ol optgroup option output p param picture pre q rp rt ruby s samp script section small source span strong style sub summary sup table tbody td tfoot th thead time title tr track u ul wbr".split(" ");
...
TagData.prototype.js = function (){
	...
	if (val instanceof Access) {
		...
		if (val instanceof PropertyAccess) {
			pars.push('[]');
		};
...
Tag.prototype.factory = function (){
	return this.scope__().imbaRef('createElement');
};
...
Tag.prototype.cacher = function (){
	var tagmap, op, op1;
	var o = this._options;
	return (o.cacher == null) ? (o.cacher = (o.key && this.parent() && false) ? (
		// declare 
		tagmap = this.scope__().imbaRef('createTagMap'),
		// if parent childCacher is declared inline this will not work(!)
		op = OP('||=',OP('.',this.parent().childCacher(),'$$'),LIT('{}')),
		new TagCache(this,this._tagScope.closure().declare("$",op,{system: true,type: 'let'}))
	) : (this.parent() ? (
		this.parent().childCacher()
	) : ((o.ivar || o.key) ? (
		op1 = OP('||=',OP('.',new This(),'$$'),LIT('{}')),
		// MAKE SURE WE REFER TO THE OUTER
		new TagCache(this,this._tagScope.closure().declare("$",op1,{system: true,type: 'let'}))
	) : (
		null
	)))) : o.cacher;
};
...
Tag.prototype.js = function (jso){
	...
	if (body) {
		var target = (o.optim && contentType == 2) ? statics : calls;
		
		// cache the body setter itself
		if (this.isSelf() && o.optim && contentType == 2 && (content instanceof TagTree)) {
		...
		if (bodySetter == 'setChildren' || bodySetter == 'setContent') {
			...
			var typ1 = (content instanceof Str) ? statics : calls;
			typ1.push(("." + bodySetter + "(" + body + ")"));
	...
	if (len$(specials)) {
		calls.push.apply(calls,specials);
	};
	...
	if (shouldEnd || this._children.length) {
		...
		if (!shouldEnd && o.optim && o.optim != this) {
			...
			if ((len$(commits) && o.optim) || !(this.isNative()) || hasAttrs || o.template) {
				calls.push(("." + (this.isSelf() ? "synced" : "end") + "(" + args + ")"));
			};
		};
	};
	...
	if (calls != statics) {
		if (o.optim && o.optim != this) {
			if (calls.length && o.commit == undefined) { this.set({commit: ("" + (o.path) + calls.join(""))}) };
		} else {
			out = ("(" + out + ")") + calls.join("");
		};
	};
...
RootScope.prototype.dump = function (){
	var obj = {warnings: AST.dump(this._warnings)};
